...
- 构建阶段会按域族合并发布条目，例如将多个 `openai.com`、`anthropic.com`、`claude.com`、`cursor.sh` 相关主机合并为对应的发布条目
- 空行和以 `#` 开头的注释会被忽略
...
- `anthropic.txt`
  - `https://raw.githubusercontent.com/ppmina/AI-ProxyRules/release/anthropic.txt`
  - `https://cdn.jsdelivr.net/gh/ppmina/AI-ProxyRules@release/anthropic.txt`
- `cursor.txt`
...
- `openai.txt`
  - `https://raw.githubusercontent.com/ppmina/AI-ProxyRules/release/openai.txt`
  - `https://cdn.jsdelivr.net/gh/ppmina/AI-ProxyRules@release/openai.txt`
- `ai.txt`
...
当前已提供 `anthropic.txt`、`cursor.txt`、`openai.txt` 三个提供方文件；后续新增其他提供方文件时，发布地址遵循相同命名规则。

## GitHub Releases

- 每次工作流发布时都会创建一个新的 GitHub Release
- Release 附件会包含当前构建生成的所有 TXT 文件，例如 `anthropic.txt`、`cursor.txt`、`openai.txt` 和 `ai.txt`
- 适合在 GitHub 网页端直接下载文件

## 源文件与构建

- 源文件目录：`source/providers/`
- 每个文件对应一个提供方，例如 `anthropic.txt`、`cursor.txt`、`openai.txt`
